...
if(!localStorage.getItem('Token')){
  ...
  useEffect(() => {
   ...
   useEffect(()=>{
      ...
      const product = await api.get(`/products/productdetail/${productId}`);
...
fetchproductseller();
  ...
  const sendMsg = async () => {
    try {
      ...
      await api.post("/chats/sendmessage", data);
 ...
 useEffect(() => {
  ...
  const getChat = async (userid, name) => {
    ...
    try {
      setMessages([]);
      const res = await api.get(`/chats/getmessage/${productId}/${userid}`);
  ...
  useEffect(() => {
    const getParticipants = async () => {
      try {
        const res = await api.get(`/chats/allparticipants/${productId}`);
